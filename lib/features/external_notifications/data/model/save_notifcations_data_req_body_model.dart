// To parse this JSON data, do
//
//     final saveNotifcationsDataReqBodyModel = saveNotifcationsDataReqBodyModelFromJson(jsonString);

import 'package:freezed_annotation/freezed_annotation.dart';
import 'dart:convert';

part 'save_notifcations_data_req_body_model.freezed.dart';
part 'save_notifcations_data_req_body_model.g.dart';

SaveNotifcationsDataReqBodyModel saveNotifcationsDataReqBodyModelFromJson(
        String str) =>
    SaveNotifcationsDataReqBodyModel.fromJson(json.decode(str));

String saveNotifcationsDataReqBodyModelToJson(
        SaveNotifcationsDataReqBodyModel data) =>
    json.encode(data.toJson());

@freezed
class SaveNotifcationsDataReqBodyModel with _$SaveNotifcationsDataReqBodyModel {
  const factory SaveNotifcationsDataReqBodyModel({
    @JsonKey(name: "device_id") String? deviceId,
    @JsonKey(name: "governorate_id") int? governorateId,
    @JsonKey(name: "fcm_token") String? fcmToken,
  }) = _SaveNotifcationsDataReqBodyModel;

  factory SaveNotifcationsDataReqBodyModel.fromJson(
          Map<String, dynamic> json) =>
      _$SaveNotifcationsDataReqBodyModelFromJson(json);
}
